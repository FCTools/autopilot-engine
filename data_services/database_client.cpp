...
namespace
{
    ...
    std::string get_zones_param_number_in_binom(const std::string ts)
    {
        ...
        std::string query("SELECT * from bot_manager_trafficsource WHERE name='" + ts + "'");
    ...
    std::string get_ts_name(const size_t ts_id)
    {
        ...
        std::string query("SELECT * from bot_manager_trafficsource WHERE id='" + std::to_string(ts_id) + "'");
        pqxx::result res = xact.exec(query);
    ...
    std::unordered_map<std::string, std::string> get_bot_info(const size_t bot_id)
    {
        ...
        std::string query("SELECT * from bot_manager_bot WHERE ID='" + std::to_string(bot_id) + "'");
        ...
        std::string ignored_zones = (bot_info + bot_indexes::IGNORED_ZONES_INDEX)->c_str();
        if (ignored_zones.size() > 0 && ignored_zones.find('\r') != std::string::npos)
        {
        ...
        return {{"condition", (bot_info + bot_indexes::CONDITION_INDEX)->c_str()},
                ...
                {"client_id", (bot_info + bot_indexes::CLIENT_ID)->c_str()},
                {"campaigns", (bot_info + bot_indexes::CAMPAIGNS_INDEX)->c_str()},
                {"tracker", (bot_info + bot_indexes::TRACKER_INDEX)->c_str()},
                {"tracker_api_key", (bot_info + bot_indexes::TRACKER_API_KEY_INDEX)->c_str()},
                {"tracker_url", (bot_info + bot_indexes::TRACKER_URL_INDEX)->c_str()}};
    }
    ...
    std::vector<std::pair<std::string, std::string>> get_bot_campaigns(std::string campaigns_json_str)
    {
        ...
        // removing spaces
        campaigns_json_str.erase(remove_if(campaigns_json_str.begin(), campaigns_json_str.end(), isspace),
        ...
        // extract all (tracker id, source id) pairs from raw json from DB
